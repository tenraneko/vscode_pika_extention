...
export const getSelectedText = (editor: TextEditor): Selection => {
  const emptySelection = new Selection(editor.document.positionAt(0), editor.document.positionAt(0));
  ...
  let widgetStartIndex = openBracketIndex > 1 ? openBracketIndex - 1 : editor.selection.anchor.character;
  for (widgetStartIndex; widgetStartIndex > 0; widgetStartIndex--) {
    const currentChar = lineText.charAt(widgetStartIndex);
    const isBeginningOfWidget = currentChar === openBracket || (currentChar === " " && lineText.charAt(widgetStartIndex - 1) !== ",");
  ...
  if (openBracketIndex < 0) {
    ...
    const endIndex = commaIndex >= 0 ? commaIndex : bracketIndex >= 0 ? bracketIndex : lineText.length;

    return new Selection(new Position(line.lineNumber, widgetStartIndex), new Position(line.lineNumber, endIndex));
  }
  ...
  for (let l = line.lineNumber; l < editor.document.lineCount; l++) {
    ...
    for (c; c < currentLine.text.length; c++) {
      ...
      if (bracketCount === 0) {
        return new Selection(new Position(line.lineNumber, widgetStartIndex), new Position(l, c + 1));
      }
    }
  }
